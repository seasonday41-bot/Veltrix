...
const YEAR_WIN='1476';
const DAY_BONUS=.20,YEAR_BONUS=.30;
...
function calendarBonusFor(dateLike,dayWinOverride=''){
  const targetDate=String(dateLike||thaiTodayISO()).slice(0,10);
  const dayWin=normalizeWin(dayWinOverride);
  const bonus=Object.fromEntries(DIGITS.map(d=>[d,(dayWin.includes(d)?DAY_BONUS:0)+(YEAR_WIN.includes(d)?YEAR_BONUS:0)]));
  return {targetDate,dayWin,yearWin:YEAR_WIN,dayBonus:DAY_BONUS,yearBonus:YEAR_BONUS,bonus};
}
...
function rankMode(rows,mode,rud,targetDate,dayWinOverride){
  ...
  const baseScore={},calendar=calendarBonusFor(targetDate,dayWinOverride),score={};
  for(const d of DIGITS){
    const gap=gapAny(d,rows),rudHit=(d===rud.top||d===rud.bottom)?1:0;
    if(P.size<=7)baseScore[d]=(P.has(d)?sat(P.size):0)+.5*gap;
    else if(P.size===8)baseScore[d]=.25*(P.has(d)?1:0)+.10*pFreq[d]+.20*persistence[d]+.05*recency[d]+.10*(S.has(d)?1:0)+.25*gap+.05*rudHit;
    else if(P.size===9)baseScore[d]=(P.has(d)?sat(P.size):0)+.5*(S.has(d)?sat(S.size):0)+.5*gap;
    else baseScore[d]=(S.has(d)?sat(S.size):0)+.5*gap;
    score[d]=baseScore[d]+calendar.bonus[d];
  }
  const ranked=[...DIGITS].sort((a,b)=>score[b]-score[a]||baseScore[b]-baseScore[a]||(+a)-(+b));
  const A=pool(rows.slice(0,3)),B=pool(rows.slice(2,5));
  return {ranked,score,baseScore,calendar,A:[...A].sort().join(''),B:[...B].sort().join(''),poolSize:P.size};
}
...
function sharedWin7(rows,rank,rud,mode){
  const out=[];
  const {primary,support}=modeRows(rows,mode);
  const primaryPool=pool(primary),supportPool=pool(support),seen5=pool(rows.slice(0,5));
  for(const d of rud2Digits(rud))if(d&&primaryPool.has(d)&&!out.includes(d))out.push(d);
  for(const d of rank.ranked)if(primaryPool.has(d)&&!out.includes(d)){out.push(d);if(out.length===7)break;}
  if(out.length<7)for(const d of rud2Digits(rud))if(d&&supportPool.has(d)&&!out.includes(d)){out.push(d);if(out.length===7)break;}
  if(out.length<7)for(const d of rank.ranked)if(supportPool.has(d)&&!out.includes(d)){out.push(d);if(out.length===7)break;}
  if(out.length<7)for(const d of rank.ranked)if(seen5.has(d)&&!out.includes(d)){out.push(d);if(out.length===7)break;}
...
export function calculateVeltrix(rows,options={}){
  const clean=(rows||[]).slice(0,10);if(clean.length<5)return null;
  const targetDate=options?.targetDate||thaiTodayISO(),dayWinOverride=normalizeWin(options?.dayWinOverride||''),result={};
  for(const mode of ['A','B']){
    const rud=crossRudEngine(clean,mode),rank=rankMode(clean,mode,rud,targetDate,dayWinOverride),win7=sharedWin7(clean,rank,rud,mode);
    ...
    result[mode]={
      ...
      rankScores:score,baseRankScores:rank.baseScore,
      targetDate:rank.calendar.targetDate,dayWin:rank.calendar.dayWin,yearWin:rank.calendar.yearWin,calendarBonus:rank.calendar,
      fusionBonus:null,
      hybridVersion:`MODE_${mode}_PRIMARY_FIRST_GLOBAL_DAYWIN_V11`
    };
  }
  return result;
}
