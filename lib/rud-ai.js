...
// 10 completed transitions. Recent stays important, with the strongest learning
// emphasis on transitions 3-5 as requested.
...
function allSourceEvidence(rows){
  ...
  for(const d of DIGITS){
    ...
    // The RUD rank is driven by the strongest learned source/positions, not by WIN6 order.
  ...
  return {score:normMap(score),rawScore:score,sideByDigit,evidence,current};
}
...
function buildPairs(rows,win6,rankScore){
  ...
  for(const a of win6)for(const b of win6){
    ...
    c.push({p:a+b,score:2.2*(top+bottom)+.55*ai+.35*hist});
  }
...
function buildTriples(rows,win6,rankScore,pairs){
  function posProb(pos,d){let count=1,total=10;rows.slice(0,5).forEach((r,i)=>{const w=1.5-.15*i;total+=w;if(chars(r)[pos]===d)count+=w;});return count/total;}
  const c=[];
  for(const a of win6)for(const b of win6)for(const d of win6){
    const t=a+b+d,position=posProb(0,a)*posProb(1,b)*posProb(2,d),ai=((rankScore[a]||0)+(rankScore[b]||0)+(rankScore[d]||0))/3,pairBonus=pairs.some(p=>t.includes(p[0])&&t.includes(p[1]))?1:0;
    ...
    c.push({t,score:3*position+.65*ai+.25*pairBonus+.45*hist});
  }
...
export function calculateIndependentRud(inputRows,rankScore={},win6Input=''){
  const rows=normalizeRows(inputRows);if(rows.length<5)return null;
  const p56=percent56Evidence(rows),all=allSourceEvidence(rows),win6=[...String(win6Input||'')].filter(d=>DIGITS.includes(d));
  const p56Candidates=DIGITS.filter(d=>p56.score[d]>=0).sort((a,b)=>p56.score[b]-p56.score[a]||all.score[b]-all.score[a]||Number(a)-Number(b));
  const allRank=[...DIGITS].sort((a,b)=>all.score[b]-all.score[a]||Number(a)-Number(b));
  const primary=p56Candidates[0]||allRank[0];
  const secondary=allRank.find(d=>d!==primary)||DIGITS.find(d=>d!==primary);
  const outside=DIGITS.filter(d=>!win6.includes(d));
  const p56Outside=p56Candidates.filter(d=>outside.includes(d));
  const smartReserve=p56Outside[0]||outside.sort((a,b)=>all.score[b]-all.score[a]||Number(a)-Number(b))[0]||null;
  return {
    primary,secondary,
    ...
    p56Raw:p56.raw,
    smartReserve,
    version:'RUD_AI_P56_POSITION_MARKET_V1'
  };
}
...
export function enhanceVeltrixWithRud(inputRows,outputs){
  ...
  for(const [key,p] of Object.entries(outputs)){
    const win6=[...String(p.win6||'')].slice(0,6),rud=calculateIndependentRud(rows,p.rankScores||{},win6.join(''));
    if(!rud){next[key]=p;continue;}
    const pairs=buildPairs(rows,win6,p.rankScores||{}),triples=buildTriples(rows,win6,p.rankScores||{},pairs),rank7=p.reserve7;
    next[key]={
      ...p,
      reserveRank7:rank7,
      reserve7:rud.smartReserve||rank7,
      reserveStrategy:rud.smartReserve?'RUD_AI_SMART_RESERVE':'RANK7_FALLBACK',
      rudTop:rud.primary,
      ...
      rudAI:rurSafe(rud),
      rud2:[rud.primary,rud.secondary],
      pair2Shared:pairs,
      pair2Top:pairs,
      pair2Bottom:[],
      pair3Top:triples,
      hybridVersion:`${p.hybridVersion||'VELTRIX'}+RUD_AI_SMART_RESERVE_V15`
    };
  }
  return next;
}
