...
const DAY_WIN={0:'7418529',1:'8327610',2:'9236701',3:'0145892',4:'4781256',5:'5690347',6:'6509438'};
const YEAR_WIN='1476';
...
function calendarBonusFor(dateLike){
  const targetDate=String(dateLike||thaiTodayISO()).slice(0,10),m=targetDate.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  let dayWin='';
  if(m){
...
function rankMode(rows,mode,rud,targetDate){
  ...
  const baseScore={},calendar=calendarBonusFor(targetDate),score={};
  for(const d of DIGITS){
...
export function calculateVeltrix(rows,options={}){
  ...
  const targetDate=options?.targetDate||thaiTodayISO(),result={};
  for(const mode of ['A','B']){
    const rud=crossRudEngine(clean,mode),rank=rankMode(clean,mode,rud,targetDate),win7=sharedWin7(clean,rank,rud,mode);
    ...
    result[mode]={
      ...
      fusionBonus:null,
      hybridVersion:`MODE_${mode}_PRIMARY_FIRST_BALANCED_DAY_YEAR_V10`
    };
  }
  return result;
}
