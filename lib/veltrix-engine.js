...
function sharedClusteredPairs(rows,win7,rank,rud){const rd=new Set(rud2Digits(rud)),primary=modeRows(rows,'B').primary,c=[];for(let i=0;i<win7.length;i++)for(let j=i;j<win7.length;j++){const a=win7[i],b=win7[j];if(i===j&&!primary.some(r=>r.top3.includes(a+a)||r.bottom2.includes(a+a)))continue;const topAffinity=pairAffinity(rows,a,b,'top'),bottomAffinity=pairAffinity(rows,a,b,'bottom'),sharedAffinity=topAffinity+bottomAffinity,ai=((rank.score[a]||0)+(rank.score[b]||0))/2,rudHits=(rd.has(a)?1:0)+(rd.has(b)?1:0),crossEvidence=((rud.crossScores?.[a]||0)+(rud.crossScores?.[b]||0))/2,repeatBonus=a===b?.35:0;c.push({p:`${a}${b}`,key:canonPair(`${a}${b}`),score:sharedAffinity+ai*.45+rudHits*.4+crossEvidence*.45+repeatBonus});}c.sort((x,y)=>y.score-x.score||(+x.p)-(+y.p));const out=[],seen=new Set();for(const x of c){if(seen.has(x.key))continue;seen.add(x.key);out.push(x.p);if(out.length===10)break;}return out;}
function hybridTriples(rows,win7,rank,rud,pairs){
  const {primary,support}=modeRows(rows,'B'),rd=new Set(rud2Digits(rud)),c=[];
  for(let pi=0;pi<pairs.length;pi++)for(const d of win7){
    ...
    const ai=ds.reduce((s,x)=>s+(rank.score[x]||0),0)/3,pairBase=(pairs.length-pi)/pairs.length,rudHits=unique.filter(x=>rd.has(x)).length,crossEvidence=ds.reduce((s,x)=>s+(rud.crossScores?.[x]||0),0)/3,score=1.15*pairBase+.55*ai+hist+rudHits*.35+crossEvidence*.45;
    c.push({t,key,pairKey:canonPair(pair),pair,third:d,score});
  }
  c.sort((a,b)=>b.score-a.score||a.t.localeCompare(b.t));

  const out=[],usedKeys=new Set(),pairUse=new Map(),digitUse=Object.fromEntries(DIGITS.map(d=>[d,0]));
  let bothRudCount=0;
  const choose=(strict=true)=>{
    while(out.length<5){
      ...
      for(const x of c){
        ...
        const ds=[...x.t],unique=[...new Set(ds)],local=Object.fromEntries(DIGITS.map(d=>[d,0]));
        ds.forEach(d=>local[d]++);
        ...
        const newDigits=unique.filter(d=>(digitUse[d]||0)===0).length;
        const usePenalty=unique.reduce((s,d)=>s+(digitUse[d]||0)*.28,0);
        const pairPenalty=(pairUse.get(x.pairKey)||0)*.95;
        let overlapPenalty=0;
        for(const y of out){const ys=new Set([...y]);const common=unique.filter(d=>ys.has(d)).length;if(common>=2)overlapPenalty+=(common-1)*.42;}
        const freshThird=x.pair.includes(x.third)?0:.12;
        const adj=x.score+newDigits*.48+freshThird-usePenalty-pairPenalty-overlapPenalty-(hasBothRud&&bothRudCount>=1?.35:0);
        if(adj>bestAdj||(adj===bestAdj&&best&&(x.score>best.score||(x.score===best.score&&x.t<best.t)))){best=x;bestAdj=adj;}
      }
      if(!best)break;
      usedKeys.add(best.key);out.push(best.t);pairUse.set(best.pairKey,(pairUse.get(best.pairKey)||0)+1);
      [...best.t].forEach(d=>digitUse[d]++);
...
export function calculateVeltrix(rows){const clean=(rows||[]).slice(0,10);if(clean.length<5)return null;const result={};for(const mode of ['A','B']){const rud=mode==='B'?crossRudEngine(clean):rudEngine(clean,mode),rank=rankMode(clean,mode,rud);if(mode==='B'){const win7=modeBWin7(rank,rud),p2shared=sharedClusteredPairs(clean,win7,rank,rud),p3=hybridTriples(clean,win7,rank,rud,p2shared),score={...rank.score};for(let i=0;i<win7.length;i++)score[win7[i]]=(score[win7[i]]||0)+(7-i)*1e-6;result[mode]={mode,win6:win7.slice(0,6).join(''),reserve7:win7[6],rudTop:rud.top,rudBottom:rud.bottom,rudSupport:null,rud2:rud2Digits(rud),crossRud:true,crossRudScores:rud.crossScores,pair2Shared:p2shared,pair2Top:p2shared,pair2Bottom:[],pair3Top:p3,poolA:rank.A,poolB:rank.B,poolSize:rank.poolSize,rankScores:score,baseRankScores:rank.score,fusionBonus:null,hybridVersion:'B_CROSS_RUD2_SHARED_PAIR10_DIVERSE_P3_V5'};}else{const p2=legacyPair2(clean,rank,rud,mode),p3=legacyPair3(clean,rank,rud,p2.top,mode);result[mode]={mode,win6:rank.ranked.slice(0,6).join(''),reserve7:rank.ranked[6],rudTop:rud.top,rudBottom:rud.bottom,rudSupport:rud.support,pair2Top:p2.top,pair2Bottom:p2.bottom,pair3Top:p3,poolA:rank.A,poolB:rank.B,poolSize:rank.poolSize,rankScores:rank.score,baseRankScores:rank.score,fusionBonus:null};}}return result;}
