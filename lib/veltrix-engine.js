...
// MODE B WIN Fusion V1
// Pair/Rud evidence adds a bounded bonus to the original ranking. Nothing is
// forcibly inserted into WIN6/Reserve, so the base MODE B ranking remains the anchor.
function fuseModeBWin(rank,rud,p2,p3){
  ...
  const score={},bonus={};
  for(const d of DIGITS){
    const topHit=top[d]>0?1:0, tripleHit=triples[d]>0?1:0;
...
export function calculateVeltrix(rows){
  ...
  for(const mode of ['A','B']){
    const rank=rankMode(clean,mode,rud);
    const p2=pair2Engine(clean,rank,rud);
    const p3=pair3Engine(clean,rank,rud,p2.top);
    const winRank=mode==='B'?fuseModeBWin(rank,rud,p2,p3):{ranked:rank.ranked,score:rank.score,bonus:null};
    const win6=winRank.ranked.slice(0,6).join(''),reserve7=winRank.ranked[6];
