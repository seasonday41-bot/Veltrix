...
function rowFromPrediction(marketId,sourceId,p){
  ...
  return {
    ...
    metadata:{
      ...
      pair2_layout:'coherent_win6_5_pairs_specialist',pair2_shared:shared.slice(0,5),pair2_shadow:p.pair2Shadow||null,
...
export async function ensureAutoSnapshots(items=[]){
  ...
  for(const x of unique){
    ...
    const core=calculateVeltrix(rows,{targetDate:x.draw_date,errorMemory:errorMemories.get(x.market_id)||null});
    ...
    for(const p of Object.values(predictions)){
      ...
      const pair2Shadow=calculatePair2Shadow(rows,p.win6);
      inserts.push(rowFromPrediction(x.market_id,x.previous_result_id,{...p,pair2Shadow}));createdMarkets.add(x.market_id);
    }
  }
  if(inserts.length)await db('veltrix_prediction_snapshots',{method:'POST',prefer:'return=minimal',body:inserts});
  return {created:inserts.length,markets:createdMarkets.size,skipped,engine_version:ENGINE_VERSION,world_win:worldWin,pair2_shadow:'PAIR2_V21_EXACT_DNA_3_5'};
}
