...
export async function ensureAutoSnapshots(items=[]){
  ...
  const [existing,historyRows,dailyRows]=await Promise.all([
  ...
  const globalDaily=dailyRows?.[0]?.setting_value||{};
  ...
  for(const x of unique){
    ...
    const dayWinOverride=globalDaily.date===x.draw_date?String(globalDaily.digits||''):'';
    const predictions=calculateVeltrix(rows,{targetDate:x.draw_date,dayWinOverride});
