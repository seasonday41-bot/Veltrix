...
function rankWithWeight(rows,baseWeight){
  const base=baseRanking(rows[0]),recent=recentDigitScore(rows,base.formulas),recentWeight=1-baseWeight;
  const score=Object.fromEntries(DIGITS.map(d=>[d,baseWeight*base.score[d]+recentWeight*recent.score[d]]));
  const ranked=[...DIGITS].sort((a,b)=>score[b]-score[a]||base.ranked.indexOf(a)-base.ranked.indexOf(b)||Number(a)-Number(b));
  return {base,recent,score,ranked};
}
...
function selectAdaptiveWeight(rows){
  ...
  for(let j=1;j<Math.min(rows.length-4,1+VALIDATION_WINDOW);j++){
    ...
    for(const w of ADAPTIVE_WEIGHTS){
      const ranked=rankWithWeight(sub,w).ranked.slice(0,6),result=forwardScore(ranked,target);
...
export function calculateVeltrix(inputRows,options={}){
  ...
  const adaptive=selectAdaptiveWeight(rows),rank=rankWithWeight(rows,adaptive.baseWeight),drift=driftEngine(rows);
  ...
  const prediction={
    ...
    hybridVersion:'ADAPTIVE_3_FORWARD_SCORE_WIN6_V13'
  };
