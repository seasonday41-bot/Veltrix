...
function doubleDigits(row){
  const t=top3(row.top3),b=bottom2(row.bottom2),out=[];
...
// Legacy linked Pair2 score is kept only as internal Pair3 evidence so Pair3 can
// specialize independently without changing the visible Pair2 specialist.
function buildLegacyPairEvidence(rows,win6,rankScore,rud){
  const c=[];
  for(const a of win6)for(const b of win6){
    if(a===b&&!rows.slice(0,5).some(r=>r.top3.includes(a+a)||r.bottom2===a+a))continue;
    const top=positionProb(rows,1,a)*positionProb(rows,2,b),bottom=positionProb(rows,3,a)*positionProb(rows,4,b),ai=((rankScore[a]||0)+(rankScore[b]||0))/2;
...
// Pair2 specialist: use only task-specific positional probability inside FINAL WIN6.
// This removes unrelated rank/RUD/historical bonuses that reduced exact Pair2 hits
// in the 60-market walk-forward benchmark.
function buildPair2Specialist(rows,win6){
  const c=[];
  for(const a of win6)for(const b of win6){
    if(a===b&&!rows.slice(0,5).some(r=>r.top3.includes(a+a)||r.bottom2===a+a))continue;
    const top=positionProb(rows,1,a)*positionProb(rows,2,b),bottom=positionProb(rows,3,a)*positionProb(rows,4,b);
    c.push({p:a+b,score:top+bottom});
  }
  c.sort((x,y)=>y.score-x.score||x.p.localeCompare(y.p));
  const out=[],seen=new Set();
  for(const x of c){const key=[...x.p].sort().join('');if(seen.has(key))continue;seen.add(key);out.push(x.p);if(out.length===5)break;}
...
function buildLegacyTriples(rows,win6,rankScore,rud,pairs){
  const c=[];
  for(const a of win6)for(const b of win6)for(const d of win6){
    const t=a+b+d,position=positionProb(rows,0,a)*positionProb(rows,1,b)*positionProb(rows,2,d),ai=((rankScore[a]||0)+(rankScore[b]||0)+(rankScore[d]||0))/3;
...
// Pair3 specialist: formula-produced triples proved more useful for exact 3-top
// than one universal score. Candidate order is locked from the 600-point training
// split and every digit must still be inside FINAL WIN6. Legacy linked scoring is
// only a deterministic fallback when fewer than three formula candidates survive.
function buildPair3Specialist(rows,win6,rankScore,rud,legacyPairs){
  const formulas=calculateFormulaSet(rows[0]),out=[],seen=new Set(),winSet=new Set(win6);
  const add=t=>{
    if(typeof t!=='string'||t.length!==3||[...t].some(d=>!winSet.has(d)))return;
    const key=[...t].sort().join('');
    if(seen.has(key))return;
    seen.add(key);out.push(t);
  };
  for(const name of PAIR3_FORMULA_PRIORITY){add(formulas[name]);if(out.length===3)return out;}
  for(const t of buildLegacyTriples(rows,win6,rankScore,rud,legacyPairs)){add(t);if(out.length===3)break;}
  return out;
}

// Double specialist keeps the existing probability %, but independently ranks the
// three watch digits. All watch digits remain inside FINAL WIN6.
function buildDoubleSpecialist(rows,win6,rankScore,currentFormulas){
  const hist=Object.fromEntries(DIGITS.map(d=>[d,0]));
  rows.slice(0,10).forEach((r,i)=>{const w=.86**i;for(const d of doubleDigits(r))hist[d]+=w;});
  const maxHist=Math.max(1,...win6.map(d=>hist[d]));
  const formula2=`${currentFormulas['สูตร 2']||''}${currentFormulas['สูตร 2.1']||''}${currentFormulas['สูตร 2.2']||''}`;
  const scored=win6.map((d,i)=>({
    d,
    score:.25*(hist[d]/maxHist)+.30*(formula2.includes(d)?1:0)+.30*(rankScore[d]||0)-i*1e-5
  }));
  scored.sort((a,b)=>b.score-a.score||Number(a.d)-Number(b.d));
  return {watch:scored.slice(0,3).map(x=>x.d),scores:Object.fromEntries(scored.map(x=>[x.d,x.score]))};
}
...
export function enhanceVeltrixWithRud(inputRows,outputs){
  ...
  for(const [key,p] of Object.entries(outputs)){
    ...
    const rud2=[rud.primary,rud.secondary],rankScore=p.rankScores||{},rank7=p.reserve7;
    const legacyPairs=buildLegacyPairEvidence(rows,win6,rankScore,rud2);
    const pair2=buildPair2Specialist(rows,win6);
    const pair3=buildPair3Specialist(rows,win6,rankScore,rud2,legacyPairs);
    const doubleSpecialist=buildDoubleSpecialist(rows,win6,rankScore,p.formulaOutputs||calculateFormulaSet(rows[0]));
    next[key]={
      ...p,
      // WIN6 membership is the central lock. Specialists never inject an outside digit.
      win6:win6.join(''),
      reserveRank7:rank7,
      ...
      pair2Bottom:[],
      pair3Top:pair3,
      doubleWatch:doubleSpecialist.watch,
      doubleScores:doubleSpecialist.scores,
      ...
      relationshipLocked:true,
      hybridVersion:`${p.hybridVersion||'VELTRIX'}+RUD_LINKED_RESERVE_V15+${SPECIALIST_VERSION}`
    };
  }
  return next;
}
