...
const LIVE_VERSION='adaptive_v14';
const BACKFILL_VERSION='adaptive_v14_backfill';
...
export function buildErrorMemory(detailsInput=[],options={}){
  ...
  return {
    version:'snapshot_error_memory_v1',
...
export async function loadErrorMemory(marketId){
  ...
  const versions=`(${LIVE_VERSION},${BACKFILL_VERSION})`;
  ...
  for(const a of audits||[]){
    ...
    const priority=s.engine_version===LIVE_VERSION?2:1;
...
export const ERROR_MEMORY_VERSIONS={live:LIVE_VERSION,backfill:BACKFILL_VERSION};
