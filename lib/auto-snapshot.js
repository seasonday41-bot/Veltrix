...
function rowFromPrediction(marketId,sourceId,p){
  ...
  return {
    ...
    metadata:{
      ...
      pair2_layout:'coherent_win6_5_pairs_specialist',pair2_shared:shared.slice(0,5),pair2_no_world_shadow:p.pair2NoWorldShadow||null,
...
export async function ensureAutoSnapshots(items=[]){
  ...
  const sourceIds=unique.map(x=>x.previous_result_id),marketIds=unique.map(x=>x.market_id),sourceFilter=`(${sourceIds.join(',')})`,marketFilter=`(${marketIds.join(',')})`;
  const [existing,historyRows,errorMemories,worldRows]=await Promise.all([
    ...
    loadErrorMemories(marketIds),db('veltrix_engine_settings?select=setting_value&setting_key=eq.daily_win_global&limit=1')
  ]);
  ...
  const inserts=[],createdMarkets=new Set();let skipped=0;
  for(const x of unique){
    const rows=historyByMarket.get(x.market_id)||[];if(rows.length<5||rows[0]?.id!==x.previous_result_id){skipped++;continue;}
    const errorMemory=errorMemories.get(x.market_id)||null;
    const core=calculateVeltrix(rows,{targetDate:x.draw_date,errorMemory});
    const noWorldPredictions=enhanceVeltrixWithRud(rows,core);
    ...
    for(const p of Object.values(predictions)){
      ...
      const challenger=noWorld?calculatePair2Shadow(rows,noWorld.win6):null;
      const championPairs=noWorld?(Array.isArray(noWorld.pair2Shared)?noWorld.pair2Shared:(noWorld.pair2Top||[])):[];
      const pair2NoWorldShadow=noWorld&&challenger?{
        version:PAIR2_SHADOW_TRACK,world_win:false,win6:noWorld.win6,
        champion:{version:'PAIR2_POSITION_SPECIALIST_V1',pairs:championPairs.slice(0,5)},
        challenger
      }:null;
      inserts.push(rowFromPrediction(x.market_id,x.previous_result_id,{...p,pair2NoWorldShadow}));createdMarkets.add(x.market_id);
    }
  }
  if(inserts.length)await db('veltrix_prediction_snapshots',{method:'POST',prefer:'return=minimal',body:inserts});
  return {created:inserts.length,markets:createdMarkets.size,skipped,engine_version:ENGINE_VERSION,world_win:worldWin,pair2_shadow:PAIR2_SHADOW_TRACK};
}
