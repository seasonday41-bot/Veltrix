...
export async function ensureAutoSnapshots(items=[]){
  ...
  for(const x of unique){
    ...
    const locked=lockedBySource.get(x.previous_result_id)||new Set();for(const p of Object.values(predictions)){if(locked.has(p.mode))continue;inserts.push(rowFromPrediction(x.market_id,x.previous_result_id,p));createdMarkets.add(x.market_id);}
  }
  if(inserts.length)await db('veltrix_prediction_snapshots',{method:'POST',prefer:'return=minimal',body:inserts});
  return {created:inserts.length,markets:createdMarkets.size,skipped,engine_version:ENGINE_VERSION,world_win:worldWin};
}
